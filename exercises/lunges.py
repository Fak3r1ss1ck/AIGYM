    ...
    while cap.isOpened():
        ...
        try:
            if results.pose_landmarks:
                ...
                # Get positions of key landmarks for lunges
                left_knee = landmarks[mp_pose.PoseLandmark.LEFT_KNEE.value]
                right_knee = landmarks[mp_pose.PoseLandmark.RIGHT_KNEE.value]
                left_hip = landmarks[mp_pose.PoseLandmark.LEFT_HIP.value]
                right_hip = landmarks[mp_pose.PoseLandmark.RIGHT_HIP.value]

                # Logic for detecting lunges
                if left_knee.y > left_hip.y and right_knee.y > right_hip.y:  # Down position
                    stage = "down"
                elif left_knee.y < left_hip.y and right_knee.y < right_hip.y and stage == 'down':  # Up position
                    stage = "up"
                    lunge_counter += 1

                # Display counter and stage
                cv2.rectangle(image, (10, 10), (250, 150), (245, 117, 16), -1)
                cv2.putText(image, 'LUNGES', (20, 40),
